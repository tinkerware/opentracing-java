...
public final class BraveTracerImpl extends AbstractTracer {
    ...
    public <T> void inject(Span span, T carrier) {
        brave.clientTracer().startNewSpan(((BraveSpanImpl)span).operationName);
    ...
    public <T> BraveSpanBuilderImpl join(T carrier) {
        ...
        brave.serverTracer()
                .setStateCurrentTrace(builder.traceId, builder.spanId, builder.parentSpanId, builder.operationName);
